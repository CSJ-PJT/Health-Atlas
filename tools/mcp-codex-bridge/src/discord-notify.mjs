...
function buildPmReviewMessage(review, result, screenshotPaths) {
  ...
  const screenshotLine = screenshotPaths.length
    ? `Fresh screenshots attached: ${screenshotPaths.length}`
    : "No fresh screenshot: milestone still in progress.";
  ...
  return [
    ...
    ``,
    `**First 3-second player impression**`,
    ...
    ``,
    `**Next milestone**`,
    nextTask === "None queued" ? "PM decision required before continuing." : `Candidate next task: \`${nextTask}\``,
    ``,
    `PM reply format:`,
    `approve ${taskId}`,
...
function summarizeVisualChange(review) {
  ...
  if (review.verdict === "request_changes") return "The latest builder result is not visually approved yet because the reviewer found a blocking issue.";
  return "The latest builder result is ready for PM visual judgement. Review the attached screenshots first.";
}

function summarizeApprovalReason(review, findings) {
  if (review.verdict === "request_changes") {
    const first = findings[0]?.issue || findings[0]?.evidence || "Reviewer requested changes.";
    return `Reject unless this issue is acceptable: ${first}`;
  }
  if (review.verdict === "stop") return "Approve only if the screenshot set proves the milestone; otherwise reject with the visual reason.";
  return "Approve if the attached screenshots read as a real controlled game space rather than a test map.";
}
...
function summarizeFirstImpression(review) {
  ...
  return "Judge whether a normal player immediately reads: controlled recovery outpost, clear route, authority pressure, and intentional boundaries.";
}

function summarizeRisk(findings, result, screenshotPaths) {
  if (!screenshotPaths.length) return "No explicit screenshotPaths found in the current result JSON. Do not treat older screenshots as current proof.";
  if (!findings.length) return "No reviewer findings recorded. PM should judge only the attached fresh screenshots.";
  return findings
    .slice(0, 3)
